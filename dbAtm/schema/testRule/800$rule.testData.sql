-- Start rule part
DECLARE @ruleCondition BIGINT = (SELECT MIN(conditionId) FROM [rule].[condition] WHERE [priority] = 1)
...
DECLARE @splitNameCommission BIGINT = (SELECT MIN(splitNameId) FROM [rule].[splitName] WHERE conditionId = @ruleCondition AND [name] = N'commission')
DECLARE @splitNamefee BIGINT = (SELECT MIN(splitNameId) FROM [rule].[splitName] WHERE conditionId = @ruleCondition AND [name] = N'fee')

IF @splitNameCommission IS NULL
BEGIN
    INSERT
        [rule].splitName(conditionId, name, tag)
    VALUES
...
WHEN MATCHED AND (   target.[isSourceAmount] <> source.[isSourceAmount]
    ...
    THEN
		UPDATE SET  target.[isSourceAmount] = source.[isSourceAmount],
...
WHEN MATCHED AND (   target.[minValue] <> source.[minValue]
    ...
    THEN
		UPDATE SET  target.[minValue] = source.[minValue],
                    target.[maxValue] = source.[maxValue],
                    target.[percent] = source.[percent],
