...
let posTerminalGridDefProps = fromJS({
    data: [],
    checkedRows: {}
});

export const posTerminalGrid = (state = posTerminalGridDefProps, action) => {
        ...
        if (action.type === actionTypes.FETCH) {
            if (action.result && action.result.terminals && action.result.terminals.length > 0) {
                return state
                    ...
                    .set('checkedRows', posTerminalGridDefProps.get('checkedRows'));
            }
        }
...
const defaultPosTerminalGridColumnVisibility = fromJS([
    {title: 'ID', name: 'terminalId'},
    {title: 'Name', name: 'name'},
    {title: 'Terminal Number', name: 'terminalNumber'},
    {title: 'Terminal Serial', name: 'terminalSerial'},
    {title: 'Merchant', name: 'merchantName'},
    {title: 'Current Version', name: 'currVersion'},
    {title: 'New Version', name: 'newVersion'}
...
export const posTerminalGridColumnVisibility = (state = defaultPosTerminalGridColumnVisibility, action) => {
    if (action.type === actionList.SET_CONFIG) {
        return defaultPosTerminalGridColumnVisibility.map((v) => {
            if (action.config.pos.terminals.grid.vissibleFields.indexOf(v.get('name')) >= 0) {
