...
PayshieldCodec.prototype.init = function(config) {
    ...
    this.headerFormatLen = parseInt(config.headerFormat.split('/')[0]);
    // config.headerFormat = '6/string-left-zero';
    if (this.log.info) {
...
PayshieldCodec.prototype.encode = function(data, $meta, context) {
    ...
    if (headerNo === undefined || headerNo === null) {
        headerNo = $meta.trace = ('0'.repeat(this.headerFormatLen) + context.trace).substr(-this.headerFormatLen);
        context.trace += 1;
        if (context.trace > parseInt('9'.repeat(this.headerFormatLen))) {
            context.trace = 0;
        }
    }
