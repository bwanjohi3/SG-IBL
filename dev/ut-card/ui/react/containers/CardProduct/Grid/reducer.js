...
const propInStorage = 'cardProduct';
...
export const cardProductGrid = (state = defaultState, action) => {
    switch (action.type) {
            ...
            return (action.params)
                ? state
                    .set('checkedRows', List())
                     .update('checkedRowsChangeId', (v) => (v + 1))
                : state
                    .set('checkedRows', state.getIn(['data', 'product']).toList())
                    .update('checkedRowsChangeId', (v) => (v + 1));
        ...
        case actionTypes.TOGGLE_PRODUCT_STATUS:
...
const defaultStateGridColumnVisibility = Immutable.fromJS({
    ...
    fields: [
        ...
        {name: 'embossedTypeName', title: 'Applicable For'},
        {name: 'periodicCardFeeName', title: 'Periodic Card Fee'},
        {name: 'accountLinkLimit', title: 'Account Link Limit'},
        {name: 'pinRetriesLimit', title: 'PIN Retries Limit'},
        {name: 'isActive', title: 'Status'}
    ]});

export const cardProductGridColumnVisibility = (state = defaultStateGridColumnVisibility, action) => {
