...
import {FETCH_APPLICATION_DETAILS, SET_ERRORS} from './../actionTypes';

const cardNameApplicationAccountsDefaultState = immutable.fromJS({
    id: 0,
    changeId: 0,
    data: {
        unlinked: [],
...
export const cardApplicationDetailsAccounts = (state = cardNameApplicationAccountsDefaultState, action) => {
    switch (action.type) {
        case FETCH_APPLICATION_DETAILS:
            if (action.methodRequestState === 'finished' && action.result) {
                ...
                let accounts = actionResultImmutable.get('accountLink').map((account) => {
                    return immutable.Map({
                        key: account.get('accountLinkId'),
                        name: account.get('name')
                    });
                });
                let data = actionResultImmutable
                    .get('accounts')
                ...
                return state
                    .set('errors', cardNameApplicationAccountsDefaultState.get('errors'))
                    .set('data', data)
                    .set('linkedAs', accounts)
                    .set('remoteData', data);
            }
            return state;
        ...
        case actionTypes.CLEAR:
            return cardNameApplicationAccountsDefaultState;
        case SET_ERRORS:
