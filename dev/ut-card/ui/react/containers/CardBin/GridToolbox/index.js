...
export const ToolboxFilters = connect(
    (state) => {
        let hasCheckedItems = state.cardBinsGrid.get('checkedRows').size > 0;

        return {
            opened: hasCheckedItems ? state.cardBinsToolbox.getIn(['filters', 'opened']) : true,
...
export const ToolboxButtons = connect(
    (state) => {
        let hasCheckedItems = state.cardBinsGrid.get('checkedRows').size > 0;
        return {
            opened: hasCheckedItems ? state.cardBinsToolbox.getIn(['buttons', 'opened']) : false,
            title: 'Show Filters',
            isTitleLink: hasCheckedItems
        };
    },
