CREATE PROCEDURE  [card].[documentCard.edit]  -- this SP edits documents and attachments in cards
    @cardId bigint, --a table that holds information which document to which card is related
...
BEGIN TRY
    ...
    when not matched by source and dd.documentId in (select documentId from [card].documentCard where cardId = @cardId ) then
    ...
    ;merge into [card].documentCard da
    using
    (
        select * from @documentResult
    ) as dr on da.cardId = @cardId and da.documentId = dr.documentId
    when matched and dr.statusid = 'deleted' then
        delete
    when not matched by target and  dr.statusid != 'deleted' then
        insert  (cardId, DocumentId)
        values (@cardId, dr.documentId);
