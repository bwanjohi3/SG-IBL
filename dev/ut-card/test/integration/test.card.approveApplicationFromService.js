...
const PRODUCTNAME = 'appas' + cardConstants.PRODUCTNAME;
const USERNAME = 'appas' + userConstants.USERNAME;
...
const BIN1 = parseInt('18' + commonFunc.generateRandomFixedInteger(4));
...
var typeIdOwn, typeNameOwn, issuerId, cardNumberConstructionId, cipher, ownBin;

module.exports = function(opt, cache) {
    test({
        type: 'integration',
        name: 'Approved status update application from service',
        server: opt.server,
        ...
        steps: function(test, bus, run) {
            return run(test, bus, [userMethods.login('login', userConstants.ADMINUSERNAME, userConstants.ADMINPASSWORD, userConstants.TIMEZONE),
                ...
                commonFunc.createStep('card.application.statusUpdate', 'update application - status ACTIONSAVE unsuccessfully', context => {
                    return {
                        application: {
                            applicationId: context['add application'].cardApplication[0].applicationId,
                            customerNumber: context['list accountIds'].account[0].customerNumber,
