...
const PRODUCTNAME = 'appda' + cardConstants.PRODUCTNAME;
const USERNAME = 'appda' + userConstants.USERNAME;
...
const BIN1 = parseInt('23' + commonFunc.generateRandomFixedInteger(4));
...
module.exports = function(opt, cache) {
    test({
        type: 'integration',
        name: 'Approve decline status update application',
        server: opt.server,
        ...
        steps: function(test, bus, run) {
            return run(test, bus, [userMethods.login('login', userConstants.ADMINUSERNAME, userConstants.ADMINPASSWORD, userConstants.TIMEZONE),
                ...
                cardMethods.updateApplicationState('update application - set status to ACTIONDECLINE successfully', context => {
                    return {
                        application: {
                            applicationId: context['add application'].cardApplication[0].applicationId,
                            customerNumber: context['add customer'].customer.customerNumber,
