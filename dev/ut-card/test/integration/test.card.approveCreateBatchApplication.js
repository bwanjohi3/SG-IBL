...
const PRODUCTNAME = 'acba' + cardConstants.PRODUCTNAME;
const CUSTOMERNAME = 'AUTOMATIONTEST' + commonFunc.generateRandomNumber().toString();
const USERNAME = 'acba' + userConstants.USERNAME;
...
const BIN1 = parseInt('19' + commonFunc.generateRandomFixedInteger(4));
...
module.exports = function(opt, cache) {
    test({
        type: 'integration',
        name: 'Approve create batch status update application',
        server: opt.server,
        ...
        steps: function(test, bus, run) {
            return run(test, bus, [userMethods.login('login', userConstants.ADMINUSERNAME, userConstants.ADMINPASSWORD, userConstants.TIMEZONE),
                ...
                commonFunc.createStep('card.application.get', 'get updated application', context => {
                    ...
                    assert.equals(result.application[0].targetBranchId, orgId1, 'return updated org');
                }),
                cardMethods.updateApplicationState('update application - set status to ACTIONCREATEBATCH successfully', context => {
                    return {
                        application: {
                            applicationId: context['add application'].cardApplication[0].applicationId,
                            customerNumber: context['add customer'].customer.customerNumber,
                ...
                commonFunc.createStep('card.application.statusUpdate', 'update application - status ACTIONAPPROVE unsuccessfully', context => {
                    return {
                        application: {
                            applicationId: context['add application'].cardApplication[0].applicationId,
                            customerNumber: context['add customer'].customer.customerNumber,
                ...
                cardMethods.updateApplicationState('update application - set status of second application to Approve successfully', context => {
                    return {
                        application: {
                            ...
                            productId: context['add product successfully'].cardProduct[0].productId,
                            typeId: typeIdOwn,
                            reasonId: context['add second application'].cardApplication[0].reasonId,
                            makerComments: context['add second application'].cardApplication[0].makerComments,
                ...
                commonFunc.createStep('card.application.get', 'get updated second application', context => {
                    return {
                        applicationId: context['add second application'].cardApplication[0].applicationId
                    };
                }, (result, assert) => {
                ...
                commonFunc.createStep('card.application.statusUpdate', 'update application - set status to ACTIONCREATEBATCH - missing batchName', context => {
                    return {
                        application: {
                            applicationId: context['add application'].cardApplication[0].applicationId,
                            customerNumber: context['add customer'].customer.customerNumber,
                    ...
                    assert.equals(error.type, 'PortHTTP', 'return PortHTTP - batchName is required');
                }),
                commonFunc.createStep('card.application.statusUpdate', 'update application - set status to ACTIONCREATEBATCH - missing targetBranchId', context => {
                    return {
                        application: {
                            applicationId: context['add application'].cardApplication[0].applicationId,
                            customerNumber: context['add customer'].customer.customerNumber,
                    ...
                    assert.equals(error.type, 'PortHTTP', 'return PortHTTP - targetBranchId is required');
                }),
                commonFunc.createStep('card.application.statusUpdate', 'update application - set status to ACTIONCREATEBATCH - missing params for batch', context => {
                    return {
                        application: {
                            applicationId: context['add application'].cardApplication[0].applicationId,
                            customerNumber: context['add customer'].customer.customerNumber,
                    ...
                    assert.equals(error.type, 'PortHTTP', 'return PortHTTP - targetBranchId and batchName are required');
                }),
                commonFunc.createStep('card.application.statusUpdate', 'update application - set status to ACTIONCREATEBATCH - empty string for batchName', context => {
                    return {
                        application: {
                            applicationId: context['add application'].cardApplication[0].applicationId,
                            customerNumber: context['add customer'].customer.customerNumber,
                    ...
                    assert.equals(error.type, 'PortHTTP', 'return PortHTTP - batchName length must be at least 1 char long');
                }),
                commonFunc.createStep('card.application.statusUpdate', 'update application - set status to ACTIONCREATEBATCH - empty string for targetBranchId', context => {
                    return {
                        application: {
                            applicationId: context['add application'].cardApplication[0].applicationId,
                            customerNumber: context['add customer'].customer.customerNumber,
                    ...
                    assert.equals(error.type, 'PortHTTP', 'return PortHTTP - targetBranchId must be a number');
                }),
                commonFunc.createStep('card.application.statusUpdate', 'update application - set status to ACTIONCREATEBATCH - invalid batchName', context => {
                    return {
                        application: {
                            applicationId: context['add application'].cardApplication[0].applicationId,
                            customerNumber: context['add customer'].customer.customerNumber,
                    ...
                    assert.equals(error.type, 'PortHTTP', 'return PortHTTP - batchName must be a string');
                }),
                commonFunc.createStep('card.application.statusUpdate', 'update application - set status to ACTIONCREATEBATCH - invalid targetBranchId', context => {
                    return {
                        application: {
                            applicationId: context['add application'].cardApplication[0].applicationId,
                            customerNumber: context['add customer'].customer.customerNumber,
                        ...
                        batch: {
                            batchName: NAMEBATCH,
                            targetBranchId: cardConstants.TEST1
                        }
                    };
                }, null, (error, assert) => {
                    assert.equals(error.type, 'PortHTTP', 'return PortHTTP - targetBranchId must be a number');
                ...
                commonFunc.createStep('card.application.statusUpdate', 'update application - set status to ACTIONCREATEBATCH - missing targetBranchId', context => {
                    ...
                    assert.equals(error.type, 'PortHTTP', 'return PortHTTP - targetBranchId must be larger than or equal to 1');
                }),
                commonFunc.createStep('card.application.statusUpdate', 'update application - set status to ACTIONCREATEBATCH - missing batch', context => {
                    return {
                        application: {
                            applicationId: context['add application'].cardApplication[0].applicationId,
                            customerNumber: context['add customer'].customer.customerNumber,
                        ...
                        applicationActionId: context['list statuses'].Application.find(element => element.actionName === ACTIONCREATEBATCH).actionId
                        // batch: { batchName: NAMEBATCH, targetBranchId: context['get admin details'].memberOF[0].object }
                    };
                }, null, (error, assert) => {
                    assert.equals(error.type, 'portSQL', 'return PortHTTP - card.application.statusUpdate.thisActionIsNotAllowedForApplicationsInThisStatus');
                }),
                commonFunc.createStep('card.application.get', 'get updated second application', context => {
                    return {
                        applicationId: context['add second application'].cardApplication[0].applicationId
                ...
                commonFunc.createStep('card.application.statusUpdate', 'missing permission', context => {
                    return {
                        application: {
                            customerName: CUSTOMERNAME + '2 ' + CUSTOMERNAME,
                            personName: CUSTOMERNAME + '2 ' + CUSTOMERNAME,
                            applicationId: context['add application'].cardApplication[0].applicationId,
                            customerNumber: context['add customer'].customer.customerNumber,
                            issuingBranchId: context['add application'].cardApplication[0].issuingBranchId,
                            holderName: cardConstants.HOLDERNAME,
