...
const USERNAME = 'dnnb' + userConstants.USERNAME;
...
const PRODUCTNAME = 'dnnb' + cardConstants.PRODUCTNAME;
    ...
    test({
        ...
        steps: function(test, bus, run) {
            return run(test, bus, [userMethods.login('login', userConstants.ADMINUSERNAME, userConstants.ADMINPASSWORD, userConstants.TIMEZONE),
                ...
                commonFunc.createStep('card.batch.statusUpdate', 'edit no name batch - empty string reasonId', context => {
                    return {
                        batch: {
                            batchId: context['add no name batch'].batch[0].batchId,
                            namedBatch: NAMEDBATCHFALSE,
                    ...
                    assert.equals(error.type, 'PortHTTP', 'return joi failure reasonId must be a number');
                }),
                commonFunc.createStep('card.batch.statusUpdate', 'edit no name batch - string reasonId', context => {
                    return {
                        batch: {
                            batchId: context['add no name batch'].batch[0].batchId,
                            namedBatch: NAMEDBATCHFALSE,
                ...
                commonFunc.createStep('card.batch.statusUpdate', 'edit no name batch - invalid number reasonId', context => {
                    return {
                        batch: {
                            batchId: context['add no name batch'].batch[0].batchId,
                            namedBatch: NAMEDBATCHFALSE,
                    ...
                    assert.equals(error.type, 'PortHTTP', 'return joi failure reasonId must be larger than or equal to 1');
                }),
