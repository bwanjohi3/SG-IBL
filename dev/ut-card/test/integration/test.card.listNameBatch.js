...
const PRODUCTNAME = 'lnb' + cardConstants.PRODUCTNAME;
const TEST1 = cardConstants.TEST1;
const USERNAME = 'lnb' + userConstants.USERNAME;
const PERMISSION = 'card.batch.list';
...
const STATUSNAMEAPPROVED = 'Approved';
const STATUSNAMECOMPLETED = 'Completed';
...
const BIN1 = parseInt('41' + commonFunc.generateRandomFixedInteger(4));
    ...
    test({
        ...
        steps: function(test, bus, run) {
            return run(test, bus, [userMethods.login('login', userConstants.ADMINUSERNAME, userConstants.ADMINPASSWORD, userConstants.TIMEZONE),
                ...
                commonFunc.createStep('card.batch.list', 'list no name batches', context => {
                    return {
                        embossedTypeId: 2
                    };
                }, (result, assert) => {
                    assert.true(result[0].find(result => result.batchId === batchId), 'return correct batchId');
                    assert.equals(cardJoiValidation.validateListBatch(result[0]).error, null, 'return all batches');
                }),
                commonFunc.createStep('card.batch.list', 'list batch - missing embossedTypeId', context => {
                    return {};
                }, null, (error, assert) => {
                    assert.equals(error.type, 'PortHTTP', 'return joi failure embossedTypeId is required');
                }),
                commonFunc.createStep('card.batch.list', 'list batch - non-existing embossedTypeId', context => {
                    return {
                        embossedTypeId: 0
                    };
                }, (result, assert) => {
                    assert.same(result[0], [], 'return empty string');
                }),
                commonFunc.createStep('card.batch.list', 'list batch - empty string embossedTypeId', context => {
                    return {
                        embossedTypeId: ''
                    };
                }, null, (error, assert) => {
                    assert.equals(error.type, 'PortHTTP', 'return joi failure embossedTypeId must be a number');
                }),
                commonFunc.createStep('card.batch.list', 'list batch - invalid embossedTypeId', context => {
                    return {
                        embossedTypeId: TEST1
                    };
                }, null, (error, assert) => {
                    assert.equals(error.type, 'PortHTTP', 'return joi failure embossedTypeId must be a number');
                }),
                ...
                commonFunc.createStep('card.batch.list', 'missing permission', context => {
                    return {
                        embossedTypeId: 1
                    };
                }, null, (error, assert) => {
