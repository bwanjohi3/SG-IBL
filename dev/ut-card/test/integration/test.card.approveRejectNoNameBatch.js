...
const USERNAME = 'apprnnb' + userConstants.USERNAME;
...
const PRODUCTNAME = 'apprnnb' + cardConstants.PRODUCTNAME;
...
module.exports = function(opt, cache) {
    test({
        type: 'integration',
        name: 'Approve Reject no name Batch',
        server: opt.server,
        ...
        steps: function(test, bus, run) {
            return run(test, bus, [userMethods.login('login', userConstants.ADMINUSERNAME, userConstants.ADMINPASSWORD, userConstants.TIMEZONE),
                ...
                commonFunc.createStep('card.batch.get', 'get batch after Reject', context => {
                    return {
                        batchId: context['add no name batch'].batch[0].batchId
                    };
                }, (result, assert) => {
