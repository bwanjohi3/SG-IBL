...
const PRODUCTNAME = 'ga' + cardConstants.PRODUCTNAME;
const USERNAME = 'ga' + userConstants.USERNAME;
const PERMISSION = 'card.application.get';
...
const BIN1 = parseInt('35' + commonFunc.generateRandomFixedInteger(4));
    ...
    test({
        ...
        steps: function(test, bus, run) {
            return run(test, bus, [userMethods.login('login', userConstants.ADMINUSERNAME, userConstants.ADMINPASSWORD, userConstants.TIMEZONE),
                ...
                cardMethods.getApplication('get application successfully', context => context['add application'].cardApplication[0].applicationId),
                commonFunc.createStep('card.application.get', 'get application unsuccessfully - missing params', (context) => {
                    return {};
                }, null, (error, assert) => {
                    assert.equals(error.type, 'PortHTTP', 'applicationId is required');
                }),
                commonFunc.createStep('card.application.get', 'get application unsuccessfully - invalid applicationId', (context) => {
                    return {
                        applicationId: 0
                    };
                }, null, (error, assert) => {
                    assert.equals(error.type, 'PortHTTP', 'applicationId must be larger than or equal to 1');
                }),
                commonFunc.createStep('card.application.get', 'get application unsuccessfully - empty string applicationId', (context) => {
                    return {
                        applicationId: ''
                    };
                }, null, (error, assert) => {
                    assert.equals(error.type, 'PortHTTP', 'applicationId must be a number');
                }),
                commonFunc.createStep('card.application.get', 'get application unsuccessfully - word for applicationId', (context) => {
                    return {
                        applicationId: TEST1
                    };
                }, null, (error, assert) => {
                ...
                commonFunc.createStep('card.application.get', 'missing permission', context => {
                    return {
