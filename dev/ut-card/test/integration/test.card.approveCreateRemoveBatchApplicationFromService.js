...
const PRODUCTNAME = 'acras' + cardConstants.PRODUCTNAME;
const USERNAME = 'acras' + userConstants.USERNAME;
...
const BIN1 = parseInt('22' + commonFunc.generateRandomFixedInteger(4));
const TYPEOWN = 'own';
...
var cipher, cardNumberConstructionId, ownBin, cardBrandId, issuerId, typeIdOwn, typeNameOwn;

module.exports = function(opt, cache) {
    test({
        type: 'integration',
        name: 'Remove From Batch status update application from service',
        server: opt.server,
        ...
        steps: function(test, bus, run) {
            return run(test, bus, [userMethods.login('login', userConstants.ADMINUSERNAME, userConstants.ADMINPASSWORD, userConstants.TIMEZONE),
                ...
                commonFunc.createStep('card.type.add', 'add card type own with required fields only', context => {
                    ...
                    typeNameOwn = result.cardType[0].typeName;
                ...
                cardMethods.updateApplicationState('update application - set status to ACTIONREMOVEFROMBATCH successfully', context => {
                    return {
                        application: {
                            applicationId: context['add application'].cardApplication[0].applicationId,
                            customerNumber: opt.customerNumber,
                ...
                commonFunc.createStep('card.application.get', 'get updated application after set status to ACTIONREMOVEFROMBATCH', context => {
                    return {
                        applicationId: context['add application'].cardApplication[0].applicationId
                    };
                }, (result, assert) => {
                    assert.same(result.application[0].batchId, null, 'return empty batchId - no batch attached');
                    assert.equals(result.application[0].statusName, STATUSNAMEAPPROVED, 'return correct status name');
                    assert.equals(result.application[0].targetBranchId, orgId, 'return updated org');
                }),
