...
const PRODUCTNAME = 'da' + cardConstants.PRODUCTNAME;
const USERNAME = 'da' + userConstants.USERNAME;
...
const BIN1 = parseInt('26' + commonFunc.generateRandomFixedInteger(4));
...
module.exports = function(opt, cache) {
    test({
        type: 'integration',
        name: 'decline status update application',
        server: opt.server,
        ...
        steps: function(test, bus, run) {
            return run(test, bus, [userMethods.login('login', userConstants.ADMINUSERNAME, userConstants.ADMINPASSWORD, userConstants.TIMEZONE),
                ...
                cardMethods.updateApplicationState('update application - set status to Decline successfully', context => {
                    return {
                        application: {
                        ...
                        applicationActionId: context['list statuses'].Application.find(element => element.actionName === ACTIONDECLINE).actionId
                    };
                }),
                commonFunc.createStep('card.application.get', 'get updated application with status Decline', context => {
                    return {
                        applicationId: context['add application'].cardApplication[0].applicationId
                    };
                }, (result, assert) => {
                    assert.equals(result.application[0].statusName, STATUSNAMEDECLINED, 'return correct status name');
