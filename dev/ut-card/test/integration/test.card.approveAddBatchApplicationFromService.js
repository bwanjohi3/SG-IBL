...
const PRODUCTNAME = 'aabas' + cardConstants.PRODUCTNAME;
const USERNAME = 'aabas' + userConstants.USERNAME;
...
const ACTIONADDTOBATCH = cardConstants.ACTIONADDTOBATCH;
const NAMEBATCH = cardConstants.NAMEBATCH;
const TYPEOWN = 'own';
const NAME = cardConstants.RANDOMNAME;
const BIN1 = parseInt('16' + commonFunc.generateRandomFixedInteger(4));
...
var stdPolicy, orgId, batchId, embosedTypeIdName;
var typeIdOwn, typeNameOwn, issuerId, ownBin, cardNumberConstructionId, cipher;

module.exports = function(opt, cache) {
    test({
        type: 'integration',
        name: 'Approve add to batch status update application from service',
        server: opt.server,
        ...
        steps: function(test, bus, run) {
            return run(test, bus, [userMethods.login('login', userConstants.ADMINUSERNAME, userConstants.ADMINPASSWORD, userConstants.TIMEZONE),
                ...
                cardMethods.updateApplicationState('update application - set status to Approve successfully', context => {
                    return {
                        application: {
                            ...
                            personId: context['add application'].cardApplication[0].personId,
                            customerId: null
                        },
                        account: [{
                            applicationId: context['add application'].cardApplication[0].applicationId,
                ...
                commonFunc.createStep('card.application.get', 'get updated application after set status to ACTIONCREATEBATCH', context => {
                    return {
                        applicationId: context['add application'].cardApplication[0].applicationId
                    };
                }, (result, assert) => {
                    batchId = result.application[0].batchId;
                ...
                commonFunc.createStep('card.application.get', 'get updated second application after set status to approve', context => {
                    return {
                        applicationId: context['add second application'].cardApplication[0].applicationId
                    };
                }, (result, assert) => {
                ...
                cardMethods.updateApplicationState('update application - set status to ACTIONADDTOBATCH successfully', context => {
                    return {
                        application: {
                            applicationId: context['add second application'].cardApplication[0].applicationId,
                            customerNumber: opt.secondCustomerNumber,
                        ...
                        applicationActionId: context['list statuses'].Application.find(element => element.actionName === ACTIONADDTOBATCH).actionId
                    };
                }, opt.secondCustomerName, opt.secondCustomerName),
                commonFunc.createStep('card.application.get', 'get updated second application after ACTIONADDTOBATCH', context => {
                    return {
                        applicationId: context['add second application'].cardApplication[0].applicationId
                    };
                }, (result, assert) => {
                    assert.equals(result.application[0].batchName, NAMEBATCH, 'return correct batch name');
                    assert.equals(result.application[0].statusName, STATUSNAMECOMPLETED, 'return correct status name');
                    assert.equals(result.application[0].batchId, batchId, 'return updated batchId');
                }),
