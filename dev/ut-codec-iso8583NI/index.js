...
Iso8583.prototype.decode = function(buffer, $meta) {
    ...
    if (frame) {
       ...
       /* if (message.mtid === '0800' || message.mtid === '0810' || message.mtid.startsWith("18")) {
            $meta.opcode = String(message[70] || '');
            $meta.opcode = this.networkCodes[$meta.opcode] || $meta.opcode;
            message.mtid = message.mtid == '1824'? '1834' : message.mtid;
            message[39] = '000';
        } else {
        ...
        if (message.mtid === '0800' || message.mtid === '0810' || message.mtid.startsWith("18")) {
            ...
            $meta.opcode = String(message[3] || '').substr(0, 3);
        }
        $meta.trace = message[11];
        if (message.mtid && message.mtid.slice) {
            $meta.mtid = {
                '0': 'request',
                '1': (parseInt(message[39]) < 2) ? 'response' : 'error',
                '2': 'request',
                '3': (parseInt(message[39]) < 2 ) ? 'response' : 'error',
                '4': 'notification',
                '5': 'notification'
        ...
        if (message.mtid.startsWith("18")) {            
            $meta.method = 'cbl/transfer.1834';
        }else{
        ...
        if (message[this.emvTagsField]) {
            let emvTags = {};
            let emvTagsField = message[this.emvTagsField] instanceof Buffer ?
                message[this.emvTagsField].toString('hex') :
                message[this.emvTagsField];
            emv.tagsDecode(emvTagsField, emvTags);
            message = Object.assign(message, {emvTags:emvTags});
        }
