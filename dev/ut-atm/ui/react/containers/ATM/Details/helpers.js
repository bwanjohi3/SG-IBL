...
export function getCreateValidator(hiddenFields, requiredFields) {
    ...
    return []
    .concat(!hiddenFields.luno && requiredFields.luno ? {
        key: ['luno'],
        type: validationTypes.text,
        rules: [
            {type: textValidations.isRequired, errorMessage: 'Luno is required.'}
        ]
    } : [])
    ...
    .concat(!hiddenFields.branch ? [{
        ...
        rules: [
            {type: dropdownValidations.isRequired, errorMessage: 'Please select branch.'}
        ].concat(requiredFields.branch ? [{type: dropdownValidations.isRequired, errorMessage: 'Branch is required.'}] : [])
    }] : []);
};
